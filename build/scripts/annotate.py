...
FUN = {0x00:"sll",0x02:"srl",0x03:"sra",0x04:"sllv",0x06:"srlv",0x08:"jr",0x09:"jalr",
...
def cstr(data, S, va):
    for name,(addr,off,size) in S.items():
        if addr and addr <= va < addr+size and name not in (".bss",):
            i = off + (va-addr)
...
def main(p, start, n):
    ...
    for i in range(n):
        pc = start + i*4
        w = struct.unpack_from("<I", data, o+(pc-a))[0]
        ...
        if w == 0: s = "nop"
            ...
            if f == "jr": s = f"jr {R[rs]}"
            elif f == "jalr": s = f"jalr {R[rd]},{R[rs]}"
            elif f in ("sll","srl","sra"): s = f"{f} {R[rd]},{R[rt]},{sa}"
            elif f in ("mfhi","mflo"): s = f"{f} {R[rd]}"
            elif f in ("mult","multu","div","divu"): s = f"{f} {R[rs]},{R[rt]}"
            else: s = f"{f} {R[rd]},{R[rs]},{R[rt]}"
        elif op == 2: s = f"j 0x{((pc+4)&0xf0000000)|((w&0x3ffffff)<<2):08x}"
        elif op == 3: s = f"jal 0x{((pc+4)&0xf0000000)|((w&0x3ffffff)<<2):08x}"
        elif op == 4: s = (f"b 0x{pc+4+imm*4:08x}" if rs==rt==0 else f"beq {R[rs]},{R[rt]},0x{pc+4+imm*4:08x}")
        ...
        elif op == 1: s = f"{'bltz' if rt==0 else 'bgez'} {R[rs]},0x{pc+4+imm*4:08x}"
        elif op == 0x0f:
            s = f"lui {R[rt]},0x{w&0xffff:04x}"; hi[R[rt]] = w&0xffff
        elif op in OPI:
            m = OPI[op]
            s = f"{m} {R[rt]},{imm}({R[rs]})" if op >= 0x20 else f"{m} {R[rt]},{R[rs]},{imm}"
            if op in (0x09, 0x0d) and R[rs] in hi:
                va = ((hi[R[rs]]<<16) | (w&0xffff)) if op == 0x0d else ((hi[R[rs]]<<16) + imm) & 0xffffffff
                t = cstr(data, S, va)
                note = f"   ; 0x{va:08x}" + (f' "{t}"' if t else "")
                hi.pop(R[rs], None)
        else: s = f".word 0x{w:08x} (op{op:02x})"
        if op in (0x09,0x0d,0x23,0x24,0x25,0x21,0x20) and R[rt] in hi and op != 0x0f:
            hi.pop(R[rt], None)
