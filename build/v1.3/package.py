...
def package():
 ...
 meta=iso_tree(SRC);save(W/'original-iso-tree.json',meta);timestamp=meta['ota_v0/ota_update.in']['mtime'];app=SRC.read_bytes()[32768+574:32768+702].decode().strip()
 args=['xorriso','-abort_on','FAILURE','-indev',SRC,'-outdev',out,'-joliet','on','-rockridge','on','-compliance','iso_9660_level=1','-application_id',app,'-preparer_id','','-system_id','LINUX','-volid','CDROM','-iso_nowtime','='+str(timestamp),'-volume_date','c','='+str(timestamp),'-volume_date','m','='+str(timestamp),'-volume_date','f','='+str(timestamp),'-volume_date','all_file_dates','set_to_mtime']
 ...
 args+=['-alter_date_r','b','='+str(timestamp),'/','--','-alter_date_r','c','='+str(timestamp),'/','--','-commit','-end']
 save(W/'xorriso-command.json',list(map(str,args)));run(args,W/'xorriso-build.log');print('One UPT generated; final verification starting',flush=True)
 verify_final()

def verify_final():
 out=W/NAME;rr=iso_tree(out);jj=iso_tree(out,True);original=iso_tree(SRC)
 expected={p:digest(B/'v13/iso'/p) for p,r in original.items() if not r['directory'] and not p.startswith(('ota_v0/rootfs.squashfs.','ota_v0/ota_md5_rootfs.squashfs.'))}
 ...
 for p,r in rr.items():
  ...
  for k in ['mode','uid','gid','mtime','atime','ctime']:assert r.get(k)==ref.get(k),(p,k)
 ...
 for lo,hi in [(1,7),(8,40),(40,72),(318,446),(574,702)]:assert a[lo:hi]==b[lo:hi],(lo,hi)
