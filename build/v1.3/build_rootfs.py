...
import json,hashlib,stat,tarfile,shutil,datetime,struct,subprocess,re,os,copy,csv
...
def verify(img,out,tag):
 ...
 hardlinks=sorted(sorted(v) for v in groups.values() if len(v)>1);assert hardlinks==[]
 ...
 assert b['inodes']==a['inodes']+461,(a['inodes'],b['inodes'])
...
def build():
 ...
 old=json.loads((B/'v13/inventory.json').read_text());expected=copy.deepcopy(old);reasons=json.loads((W/'change-reasons.json').read_text());metadata=json.loads((REPO/'build/v1.3/full-mod-metadata.json').read_text()) if (REPO/'build/v1.3/full-mod-metadata.json').is_file() else {};timestamp=sb(SOURCE)['mkfs_time'];mtime=datetime.datetime.fromtimestamp(timestamp,datetime.timezone.utc).strftime('%Y-%m-%d %H:%M:%S')
 for p,why in reasons.items():
  ...
  expected[p]['size']=f.stat().st_size;expected[p]['sha256']=digest(f);expected[p]['mtime']=metadata.get(p,{}).get('mtime',expected[p].get('mtime',mtime))
 ...
 for p,meta in metadata.items():
  if p in expected and 'mtime' in meta: expected[p]['mtime']=meta['mtime']
